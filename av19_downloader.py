#coding: utf8
#title: AV19
#https://av19.org/

import downloader
from utils import Downloader, clean_title
from m3u8_tools import M3u8_stream
from io import BytesIO

@Downloader.register
class Downloader_av19(Downloader):
    type = 'av19'
    URLS = ['av19.org']
    single = True
    display_name = 'AV19'
    icon = 'base64:'
    REFERER = 'https://david.cdnbuzz.buzz/'

    def read(self):
        soup = downloader.read_soup(self.url,**{'referer':'https://av19.org/'})
        title = clean_title(soup.find('title').text.strip())
        if len(title) > 205:
            title = title[:205]
        html = downloader.read_html(soup.find('iframe')['src'], referer=self.REFERER)
        nu = html.find('ht', html.find('file:'))
        leter = html[nu -1: nu]
        m = lambda: M3u8_stream(html[nu: html.find(leter,nu)],referer=self.REFERER, n_thread=8, referer_seg=self.REFERER)
        try:
            m = m()
        except Exception as e:
            raise e
        thumb = BytesIO()
        nu = html.find('ht', html.find('image:',nu))
        downloader.download(html[nu: html.find(leter,nu)], buffer=thumb, referer=self.REFERER)
        self.urls.append(m)
        self.filenames[m] = title+'.mp4'
        self.setIcon(thumb)
        self.title = title

messageBox('If you are not in Korea, you need VPN')